...
function getAdjacent(lastNode, curr, grid) {
  ...
  if (lastNode.row === curr.row) {
    if (lastNode.col < curr.col) return grid[lastNode.row][lastNode.col + 1];
    else return grid[lastNode.row][lastNode.col - 1];
  }
  //vertical
  if (lastNode.col === curr.col) {
    if (lastNode.row < curr.row) return grid[lastNode.row + 1][lastNode.col];
    else return grid[lastNode.row - 1][lastNode.col];
  }
  //diag1
  if (lastNode.row + lastNode.col === curr.row + curr.col) {
    if (lastNode.row - lastNode.col < curr.row - curr.col)
      return grid[lastNode.row + 1][lastNode.col - 1];
    else return grid[lastNode.row - 1][lastNode.col + 1];
  }
  //diag2
  if (lastNode.row - lastNode.col === curr.row - curr.col) {
    if (lastNode.row + lastNode.col < curr.row + curr.col)
